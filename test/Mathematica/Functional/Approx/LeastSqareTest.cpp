...
TEST(LeastSqare, Playground) { // NOLINT
  // clang-format off
  const auto points = Collection::Array<Point<Float<R>>, 3>({{
      {  1,  0 },
      { -6,  0 },
      {  0, -6 },
  }});
...
TEST(LeastSqare, Polynomial) { // NOLINT
  ...
  const auto poly = LeastSqare::optimalPolynomial<Float<R>, 3>(points);
  for (Float<R> x = -30; x < 30; x += 0.2) { // NOLINT
