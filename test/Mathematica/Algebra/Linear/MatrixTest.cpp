...
TEST(Matrix, CreateFromMArray) { // NOLINT
  auto data = randomArray2D<T, N, M>(-B, B);
...
TEST(Matrix, Copy) { // NOLINT
  auto input = randomMatrix<T, N, M>(-B, B);
  auto copy = input;

  auto index = randomSize(N);
  auto newValue = randomVector<T, M>(-B, B);
...
TEST(Matrix, AddSub) { // NOLINT
  auto left = randomMatrix<T, N, M>(-B, B);
  auto right = randomMatrix<T, N, M>(-B, B);

  auto expectedSum = Matrix<T, N, M>([=](auto i, auto j) {
    return left[i][j] + right[i][j];
  });

  auto expectedDiff = Matrix<T, N, M>([=](auto i, auto j) {
    return left[i][j] - right[i][j];
  });
...
TEST(Matrix, MultOnScalar) { // NOLINT
  ...
  auto expected = Matrix<T, N, M>([=](auto i, auto j) { //
